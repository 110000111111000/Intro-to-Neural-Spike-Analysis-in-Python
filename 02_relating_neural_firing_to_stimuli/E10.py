...
# Optional: filter to spikes only within 0.15s after stimulus onset
df_filtered = df[df["relative_spike_time"] <= 0.15]



# Filter to spikes in first 0.15 seconds
#df_filtered = df[df["spike_time_relative_to_start"] <= 0.15]

# Plot histogram with increased bins and hue by brain area
sns.histplot(
    data=df_filtered,
    x="spike_time_relative_to_start",
    hue="brain_area",
    bins=100,
    element="step",
    stat="count",
...
plt.title("Spike Response by Brain Area (First 0.15s After Stimulus)")
plt.xlabel("Time After Stimulus Onset (s)")
plt.ylabel("Spike Count")
plt.xlim(0, 0.15)
plt.legend(title="Brain Area")
plt.show()

import seaborn as sns
import matplotlib.pyplot as plt

# Filter to spikes within the first 0.15 seconds relative to stimulus start
df_filtered = df[df["spike_time_relative_to_start"] <= 0.15]
