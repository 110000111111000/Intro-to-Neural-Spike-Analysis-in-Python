...
Install the conda environment from `environment.yml` and select is as the kernel for this notebook.

```bash
conda env create -f environment.yml
```

Then, run the cell below to download the data set for this session (this may take a while).
...
| `results = si.run_sorter(sorter_name, recording, output_folder)` | Apply a spike sorter to the `recordings`, save the results to the `output_folder` and assign the returned object to a variable `results` |
...
sorting_TDC = si.run_sorter(
    sorter_name="tridesclous2", recording=seg, output_folder="results_TDC"
)
...
:::{#exr-}
Run the spike sorter `"tridesclous2"` on the next 30 seconds of the recording `rec` and print the number of detected units (HINT: you'll either have to use a different `output_folder` or pass `remove_existing_folder=True`)
:::
:::{sol.}
```{python}
t_start = 30
...
:::{#exr-}
Run the spike sorter `"tridesclous2"` on the first 30 seconds of the recording `rec` and use the `detection` parameter with the values from @exm-detection and the `filtering` parameter with the values from @exr-exr-filtering. Then, print the number of detected units.
:::
:::{sol.}
```{python}
sorting_TDC = si.run_sorter(
