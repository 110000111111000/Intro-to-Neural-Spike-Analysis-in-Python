...
url = "https://uni-bonn.sciebo.de/s/bIDNCBs37frcli8"
fname = "allen"
...
:::{sol.}
```{python}
x, y = find_synchronous_spikes(sts)
rasterplot(sts, color="black")
plt.scatter(
    x, y, color="red", label="synchronous spikes", marker="o", facecolors="none"
)
...
| Code | Description |
| --- | --- |
| `utils.get_available_sessions()` | Get the IDs of all recorded sessions |
| `utils.load_spike_trains(session, max_dur=120, min_spikes=100)` | Load the spike trains from a given `session`, limit their duration to `120` seconds and only include neurons that spikes at least `100` times |
...
:::{#exm-}
List the brain areas recorded in one `session`.
```{python}
session = utils.get_available_sessions()[14]
print("Session", session)
```
:::

:::{#exm-}
Load the spike trains for all neurons in the anterolateral area `"AL"`, recorded during the given `session`. Limit the duration to `60` seconds and only include neurons that spiked at least `50` times. Then, print the number of returned spike trains.
```{python}
sts = utils.load_spike_trains(session, max_dur=60, min_spikes=50)
print(f"Loaded {len(sts)} spike trains")
```
:::
...
:::{#exr-}
Rerun SPADE but set the `binsize` to 4 ms and the `winlen` to 10 bins. To counterbalance the increases search window, add the parameter `min_neu=3` to only search for patterns that encompass at least `3` neurons. Then, find all patterns that are statistically significant and visualize them.
:::
:::{sol.}
```{python}
patterns = spade(
...
:::{#exr-}
(Optional) experiment with SPADE! Load the data from different sessions and different brain areas and play with the parameters of SPADE (HINT: increasing `winlen` and `binsize` increases the total number patterns detected which increases the comutational load for statistical evaluation. To counter-balance, you can increase the value of `min_occ` and `min_neu` to only analyze patterns that occurr frequently and encompass a larger number of neurons or decrease the amount of data by changing the `max_dur` parameter in `load_spike_trains`.)
:::