...
:::{#exr-}
Create a `sns.countplot` that displays the number of spikes recorded from each `"unit"` and add a `hue` to encode the `"brain_area"`.
:::
:::{.sol}
```{python}
...
:::{.sol}
```{python}
df.groupby(["brain_area", "unit_id"]).spike_time.nunique()
```
:::

:::{#exr-}
Divide the result from @exr-count by the duration of the recording to get the firing rate of every unit (HINT: you can approximate the duration as the difference between the first and last `"spike_time"`). Assign th result to a new variable `fr`.
:::
:::{.sol}
```{python}
dur = df.spike_time.max() - df.spike_time.min()
fr = df.groupby(["brain_area", "unit_id"]).spike_time.nunique() / dur
```
:::

:::{#exr-}
Use `fr.reset_index()` to create a new data frame where each row represents one unit and print it (HINT: the method does not modify `fr` so you must expliclity overwrite the original value). Then, rename the columns to label the new column with `"firing_rate"`.
:::
:::{.sol}
```{python}
fr = fr.reset_index()
fr.columns = ["unit_id", "brain_area", "firing_rate"]
```
:::
...
:::{#exr-}
Look at the graph created in @exr-mean that showed the mean firing rates for each `.brain_area`. Based on this plot, which differences would you expect to be significant? Test you expectation by performing Mann-Whitney-U-Test on the respective pair(s).
:::
```{python}
