...
df = pd.read_parquet(f"../data/allen/{session}/flash_spikes.parquet")
df.head(5)
```
:::
...
:::{.sol}
```{python}
df.groupby("unit_id").spike_time.nunique()
```
:::
...
:::{.sol}
```{python}
print(df.groupby("unit_id").spike_time.nunique().min())
print(df.groupby("unit_id").spike_time.nunique().max())
```
:::
...
:::{#exm-}
Get the spikes recorded from every unit in the primary visual cortex `"V1"`. Then, create a rasterplot displaying the spikes from each unit between `1300` and `1350` seconds.
:::
```{python}
...
:::{#exr-}
Create a rasterplot displaying the spikes from all units recorded in the rostrolateral area `"RL"` between `1400` and `1600` seconds.
:::
:::{.sol}
```{python}
...
## Statistical Comparison of Firing Rates
...
:::{#exr-}
Use the `.any()` method on the mask from @exr-mask2d to get a 1D mask that is `True` if a spike falls within a **0.5 s** interval after any stimulus. Then, use that mask to filter th `spikes` data frame and assign the filtered data frame to a new variable `spikes_in_window`. Print the `len()` of this new data frame to get the total number of spikes that fall within a **0.5 s** interval after any stimulus.
:::
:::{.sol}
```{python}
mask = spike_is_in_window.any(axis=1)
spikes_in_window = spikes[mask]
len(spikes_in_window)
```
:::

:::{#exr-fr}
From the data frame of the `spikes_in_window`, get the number of spikes produced by unit `951016662`. Then, divide this count by the length of the time window (i.e. **0.5 s**) times the number of stimuli to get the firing rate of the unit across all stimuli.

```{python}
spike_count = len(spikes_in_window[spikes_in_window["unit_id"] == 951016662])
...
:::{#exm-}
Use the newly defined function `find_spikes_in_window` to filter all spikes that happen between `tmin=0` and `tmax=0.5` seconds after any stimuli. Then, compute the firing rate of unit `951016662`. The result should be the same as in @exr-fr.
```{python}
tmin = 0
tmax = 0.5
spikes_in_window = find_spikes_in_window(spikes, stimuli, tmin, tmax)
...
:::{#exr-}
Use `find_spikes_in_window()` to filter all spikes in `spikes_v1` that occurr between **0.02 and 0.06 s** after any stimulus.
:::
:::{.sol}
```{python}
tmin = 0.02
tmax = 0.06
spikes_v1_in_window = find_spikes_in_window(spikes_v1, stimuli, tmin, tmax)
```
:::
...
:::{.sol}
```{python}
len(spikes_v1_in_window)/n_units_v1/(len(stimuli)*(tmax-tmin))
```
:::
...
spikes_al_in_window = find_spikes_in_window(spikes_al, stimuli, tmin, tmax)
len(spikes_al_in_window)/n_units_al/(len(stimuli)*(tmax-tmin))
```
:::
...
tmax = 0.1
spikes_v1_in_window = find_spikes_in_window(spikes_v1, stimuli, tmin, tmax)
print(len(spikes_v1_in_window) / n_units_v1 / (len(stimuli) * (tmax - tmin)))

spikes_al_in_window = find_spikes_in_window(spikes_al, stimuli, tmin, tmax)
print(len(spikes_al_in_window) / n_units_al / (len(stimuli) * (tmax - tmin)))
```
:::